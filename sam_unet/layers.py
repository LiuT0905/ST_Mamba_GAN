...
#
#     def forward(self, x):
#         # 输入形状: [batch_size, seq_len, channels, 128, 64]
#         batch_size, seq_len, channels, height, width = x.size()
#
#         # 将时间维度展平，逐时间步应用 CBAM
#         x = x.view(batch_size * seq_len, channels, height, width)  # [batch_size * seq_len, channels, 128, 64]
#         out = self.channel_att(x)  # 应用 Channel Attention
#         out = self.spatial_att(out)  # 应用 Spatial Attention
#         out = out.view(batch_size, seq_len, channels, height, width)  # 恢复时间维度
#         return out
