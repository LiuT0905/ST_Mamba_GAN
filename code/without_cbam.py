...
if __name__ == "__main__":
    ...
    parser.add_argument('--logfile', default='experiment/withoutCBAM/error/errlog.txt')
    ...
    optimizerD = torch.optim.Adam(netD.parameters(), lr=opt.lr*0.3, betas=(opt.beta1, 0.999))
    ...
    for epoch in range(opt.npre + 1, opt.npre + opt.niter + 1):
        for i, (images, masked_last_image, sequence_files) in enumerate(dataloader):
            ...
            if ((i + 1) % 40 == 0):
                vutils.save_image(fake.data, '%s/experiment/withoutCBAM/image/epoch_%03d_batch_%03d_fake.png' % (
                opt.outf, epoch, i), normalize=True)
                vutils.save_image(last_pm.data, '%s/experiment/withoutCBAM/image/epoch_%03d_batch_%03d_real.png' % (
                opt.outf, epoch, i), normalize=True)
        ...
        if epoch == 300:
            torch.save(netG.state_dict(), f'{opt.outf}/experiment/withoutCBAM/nets/netG_epoch_{epoch:03d}.pth')
