...
class Generator(nn.Module):
    def __init__(self, input_channels=12, output_channels=1, kernels_per_layer=2, bilinear=True, reduction_ratio=16):
...
# ----------------- Training script (keeps your logic) -----------------
if __name__ == "__main__":
    ...
    parser.add_argument('--logfile', default='experiment/withoutMamba_12/error/errlog.txt')
    ...
    for epoch in range(opt.npre + 1, opt.npre + opt.niter + 1):
        for i, (images, masked_last_image, sequence_files) in enumerate(dataloader):
            ...
            for k in range(opt.nk):
                ...
                real_labels = torch.full_like(output_real, 1, device=device)
            ...
            if ((i + 1) % 40 == 0):
                vutils.save_image(fake.data, '%s/experiment/withoutMamba_12/image/epoch_%03d_batch_%03d_fake.png' % (
                opt.outf, epoch, i), normalize=True)
                vutils.save_image(last_pm.data, '%s/experiment/withoutMamba_12/image/epoch_%03d_batch_%03d_real.png' % (
                opt.outf, epoch, i), normalize=True)
        ...
        if epoch == 300:
            torch.save(netG.state_dict(), f'{opt.outf}/experiment/withoutMamba_12/nets/netG_epoch_{epoch:03d}.pth')
