...
if __name__ == "__main__":
    ...
    parser.add_argument('--logfile', default='experiment/ST_Mamba_GAN_8/error/errlog.txt')
    parser.add_argument('--logfileD', default='experiment/ST_Mamba_GAN_8/error/errlog_D.txt')
    parser.add_argument('--logfileG', default='experiment/ST_Mamba_GAN_8/error/errlog_G.txt')
    parser.add_argument('--logfileMSE', default='experiment/ST_Mamba_GAN_8/error/errlog_M.txt')
    ...
    for epoch in range(opt.npre + 1, opt.npre + opt.niter + 1):
        for i, (images, masked_last_image, sequence_files) in enumerate(dataloader):
            ...
            if ((i + 1) % 40 == 0):
                vutils.save_image(fake.data, '%s/experiment/ST_Mamba_GAN_8/image/epoch_%03d_batch_%03d_fake.png' % (
                opt.outf, epoch, i), normalize=True)
                vutils.save_image(last_pm.data, '%s/experiment/ST_Mamba_GAN_8/image/epoch_%03d_batch_%03d_real.png' % (
                opt.outf, epoch, i), normalize=True)
        ...
        if epoch in (1,20,40,60,80,100,120,140,160,180,200,220,240,260,280,300):
            torch.save(netG.state_dict(), f'{opt.outf}/experiment/ST_Mamba_GAN_8/nets/netG_epoch_{epoch:03d}.pth')
